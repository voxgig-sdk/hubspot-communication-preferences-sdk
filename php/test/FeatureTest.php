...
// HubspotCommunicationPreferences SDK feature test
//
...
class FtCtrl extends HubspotCommunicationPreferencesControl
{
...
class FtClient
{
    ...
    private HubspotCommunicationPreferencesUtility $utility;

    public function __construct(HubspotCommunicationPreferencesUtility $utility, array $options)
    {
    ...
    public function get_utility(): HubspotCommunicationPreferencesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HubspotCommunicationPreferencesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HubspotCommunicationPreferencesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HubspotCommunicationPreferencesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HubspotCommunicationPreferencesConfig::make_feature($name);
    ...
    private static function buildUrl(HubspotCommunicationPreferencesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HubspotCommunicationPreferencesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HubspotCommunicationPreferencesResponse($fetched);
        }
    ...
    private function fail(HubspotCommunicationPreferencesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HubspotCommunicationPreferencesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
