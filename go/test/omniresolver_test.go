...
import (
	...
	omni "github.com/voxgig-sdk/hubspot-communication-preferences-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HubspotCommunicationPreferencesError); is {
				out := map[string]any{
					"name":    "HubspotCommunicationPreferencesError",
...
func omniCtx(arg any, client *sdk.HubspotCommunicationPreferencesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HubspotCommunicationPreferencesSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.HubspotCommunicationPreferencesError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.HubspotCommunicationPreferencesError{Msg: msg, Code: code}
}
