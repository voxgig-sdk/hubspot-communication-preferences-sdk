...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_LIVE") == "TRUE" ||
		os.Getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_EXPLAIN"); explain != "" {
