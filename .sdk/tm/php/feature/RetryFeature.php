...
// HubspotCommunicationPreferences SDK retry feature
...
class HubspotCommunicationPreferencesRetryFeature extends HubspotCommunicationPreferencesBaseFeature
{
    ...
    public function init(HubspotCommunicationPreferencesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HubspotCommunicationPreferencesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HubspotCommunicationPreferencesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HubspotCommunicationPreferencesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
