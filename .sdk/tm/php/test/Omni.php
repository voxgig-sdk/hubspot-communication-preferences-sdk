...
//    only - a typed HubspotCommunicationPreferencesContext there would make every ctx
...
final class HubspotCommunicationPreferencesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HubspotCommunicationPreferencesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HubspotCommunicationPreferencesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HubspotCommunicationPreferencesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HubspotCommunicationPreferencesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HubspotCommunicationPreferencesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HubspotCommunicationPreferencesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HubspotCommunicationPreferencesError(is_string($code) ? $code : '', $msg);
    }
