...
// HubspotCommunicationPreferences SDK utility: result_basic

class HubspotCommunicationPreferencesResultBasic
{
    public static function call(HubspotCommunicationPreferencesContext $ctx): ?HubspotCommunicationPreferencesResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof HubspotCommunicationPreferencesError) ? $result->err->msg : (string)$result->err;
