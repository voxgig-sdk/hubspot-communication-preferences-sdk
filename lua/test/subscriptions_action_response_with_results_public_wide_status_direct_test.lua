...
describe("SubscriptionsActionResponseWithResultsPublicWideStatusDirect", function()
  it("should direct-list-subscriptions_action_response_with_results_public_wide_status", function()
    local setup = subscriptions_action_response_with_results_public_wide_status_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-subscriptions_action_response_with_results_public_wide_status", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"subscriber_id_string01"}) do
