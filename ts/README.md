# HubspotCommunicationPreferences TypeScript SDK



The TypeScript SDK for the HubspotCommunicationPreferences API — a type-safe, entity-oriented client with full async/await support.
...
```ts
import { HubspotCommunicationPreferencesSDK } from '@voxgig-sdk/hubspot-communication-preferences'
...
```ts
try {
  const subscriptionsactionresponsewithresultspublicstatus = await client.SubscriptionsActionResponseWithResultsPublicStatus().load({
    channel: 'example_channel',
    id: 'example_id',
  })
  console.log(subscriptionsactionresponsewithresultspublicstatus)
} catch (err) {
...
```ts
// Create — returns the created SubscriptionsActionResponseWithResultsPublicStatus ENTITY (.data() for the record)
...
| `links` | `Record<string, any>` | Contains URLs related to the response, such as documentation or resources. |
...
| `links` | `Record<string, any>` | An object containing related links, where each key is a link name and each value is a URL. |
...
| `links` | `Record<string, any>` | A collection of related links associated with the operation. |
| `numErrors` | `number` | The number of errors encountered during the operation. |
...
| `links` | `Record<string, any>` | A collection of URLs linking to related resources or documentation. |
...
| `links` | `Record<string, any>` | URLs linking to related resources or documentation. |
...
| `inputs` | `any[]` | A list of strings representing the inputs for batch operations. |
| `links` | `Record<string, any>` | A collection of related links associated with the batch response. |
...
├── src/
│   ├── HubspotCommunicationPreferencesSDK.ts        # Main SDK class
