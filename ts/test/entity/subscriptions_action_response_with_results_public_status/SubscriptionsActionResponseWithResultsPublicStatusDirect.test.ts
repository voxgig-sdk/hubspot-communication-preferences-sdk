...
describe('SubscriptionsActionResponseWithResultsPublicStatusDirect', async () => {
  ...
  test('direct-load-subscriptions_action_response_with_results_public_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-subscriptions_action_response_with_results_public_status', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_COMMUNICATION_PREFERENCES_TEST_SUBSCRIPTIONS_ACTION_RESPONSE_WITH_RESULTS_PUBLIC_STATUS_ENTID']
