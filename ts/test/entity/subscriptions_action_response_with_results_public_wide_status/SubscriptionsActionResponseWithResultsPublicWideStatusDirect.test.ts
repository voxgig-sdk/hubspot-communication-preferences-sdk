...
describe('SubscriptionsActionResponseWithResultsPublicWideStatusDirect', async () => {
  ...
  test('direct-list-subscriptions_action_response_with_results_public_wide_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-subscriptions_action_response_with_results_public_wide_status', setup.live)) return
    if (skipIfMissingIds(t, setup, ["subscriber_id_string01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_COMMUNICATION_PREFERENCES_TEST_SUBSCRIPTIONS_ACTION_RESPONSE_WITH_RESULTS_PUBLIC_WIDE_STATUS_ENTID']
