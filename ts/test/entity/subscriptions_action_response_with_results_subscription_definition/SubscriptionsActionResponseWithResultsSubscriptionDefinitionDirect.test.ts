...
import { HubspotCommunicationPreferencesSDK } from '../../..'
...
describe('SubscriptionsActionResponseWithResultsSubscriptionDefinitionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new HubspotCommunicationPreferencesSDK({
  ...
  test('direct-list-subscriptions_action_response_with_results_subscription_definition', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-subscriptions_action_response_with_results_subscription_definition', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_COMMUNICATION_PREFERENCES_TEST_SUBSCRIPTIONS_ACTION_RESPONSE_WITH_RESULTS_SUBSCRIPTION_DEFINITION_ENTID']
