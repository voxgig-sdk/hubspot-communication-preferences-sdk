...
const SDK_NAME = 'HubspotCommunicationPreferencesSDK'
...
const TEST_SEED = {"entity":{"subscriptions_action_response_with_results_public_status":{"test01":{"id":"test01"}},"subscriptions_action_response_with_results_public_wide_status":{"test01":{"id":"test01"}},"subscriptions_action_response_with_results_subscription_definition":{"test01":{"id":"test01"}},"subscriptions_batch_response_public_bulk_opt_out_from_all":{"test01":{"id":"test01"}},"subscriptions_batch_response_public_status":{"test01":{"id":"test01"}},"subscriptions_batch_response_public_status_bulk":{"test01":{"id":"test01"}},"subscriptions_batch_response_public_wide_status_bulk":{"test01":{"id":"test01"}},"subscriptions_link_generation":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+HubspotCommunicationPreferencesSDK\b/.test(code) ||
    /\bHubspotCommunicationPreferencesSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HubspotCommunicationPreferencesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HubspotCommunicationPreferencesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
