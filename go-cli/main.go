// Command HubspotCommunicationPreferences-cli is a CLI + REPL for the HubspotCommunicationPreferences SDK,
...
const prompt = "hubspot-communication-preferences"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "subscriptions_action_response_with_results_public_status subscriptions_action_response_with_results_public_wide_status subscriptions_action_response_with_results_subscription_definition subscriptions_batch_response_public_bulk_opt_out_from_all subscriptions_batch_response_public_status subscriptions_batch_response_public_status_bulk subscriptions_batch_response_public_wide_status_bulk subscriptions_link_generation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HUBSPOT_COMMUNICATION_PREFERENCES_APIKEY (api key), HUBSPOT_COMMUNICATION_PREFERENCES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HUBSPOT_COMMUNICATION_PREFERENCES_APIKEY carries the API key and
	// HUBSPOT_COMMUNICATION_PREFERENCES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HUBSPOT_COMMUNICATION_PREFERENCES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HUBSPOT_COMMUNICATION_PREFERENCES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHubspotCommunicationPreferencesSDK(opts)
