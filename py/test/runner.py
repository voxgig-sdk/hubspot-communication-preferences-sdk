# HubspotCommunicationPreferences SDK test runner
...
class HubspotCommunicationPreferencesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HubspotCommunicationPreferencesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HubspotCommunicationPreferencesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HubspotCommunicationPreferencesTestRunner.getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_LIVE")
        override = HubspotCommunicationPreferencesTestRunner.getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HubspotCommunicationPreferencesTestRunner.getenv(key)
        ...
        explain = HubspotCommunicationPreferencesTestRunner.getenv("HUBSPOT_COMMUNICATION_PREFERENCES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HUBSPOT_COMMUNICATION_PREFERENCES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HubspotCommunicationPreferencesTestRunner._test_control is not None:
            return HubspotCommunicationPreferencesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HubspotCommunicationPreferencesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HubspotCommunicationPreferencesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HubspotCommunicationPreferencesTestRunner.load_test_control()
...
def load_env_local():
    HubspotCommunicationPreferencesTestRunner.load_env_local()


def env_override(m):
    return HubspotCommunicationPreferencesTestRunner.env_override(m)


def entity_data(v):
    return HubspotCommunicationPreferencesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HubspotCommunicationPreferencesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HubspotCommunicationPreferencesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HubspotCommunicationPreferencesTestRunner.load_test_control()


def live_client_options():
    return HubspotCommunicationPreferencesTestRunner.live_client_options()


def live_delay_ms():
    return HubspotCommunicationPreferencesTestRunner.live_delay_ms()
