...
def _enrich(ctxmap, ctx):
    ...
    if isinstance(spec_map, dict):
        ctx.spec = HubspotCommunicationPreferencesSpec(spec_map)

    res_map = ctxmap.get('result')
    if isinstance(res_map, dict):
        ctx.result = HubspotCommunicationPreferencesResult(res_map)
        err_map = res_map.get('err')
        if isinstance(err_map, dict):
            msg = err_map.get('message', '')
            if msg != '':
                ctx.result.err = HubspotCommunicationPreferencesError('', msg)

    resp_map = ctxmap.get('response')
    if isinstance(resp_map, dict):
        ctx.response = HubspotCommunicationPreferencesResponse(resp_map)
